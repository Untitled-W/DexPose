...
from vision.featurizer.utils.visualization import IMG_SIZE, Demo, visualize_max_xy, visualize_max_xy_linear, visualize_max_xy_list, visualize_mappings
...
def extract_ft_tensor(img: torch.Tensor, featurizer, prompt=None, dest_size=None):
    '''
    preprocess of img to `img`:
    img should be a torch.Tensor of shape (C, H, W)
    '''
    ...
    img_tensor = (img - 0.5) * 2  # Normalize to [-1, 1]
    
    ft = featurizer.forward(
        img_tensor.unsqueeze(0).cuda(),  # Add batch dimension and move to GPU
        block_index=1,  # only for clip & dino
        prompt=prompt,  # only for sd
        ensemble_size=2  # only for sd
    )
    dest_ft = F.interpolate(ft, size=dest_size, mode='bilinear', align_corners=False) if dest_size else ft
    return dest_ft
...
def map_keypoints(src_img_PIL, tgt_img_PIL, prompt, src_pos_list, save_root=None, ftype='sd'):
    """
    Resize the image to (IMG_SIZE, IMG_SIZE) and extract features
    :param src_img_PIL: source image
    ...
    for src_pos in src_pos_list:
        ...
        relevance_map_ls.append(cos_map.squeeze(0))
    src_pos_list_np = np.array(src_pos_list)
    max_xy_list_np = np.array(max_xy_list)
    visualize_mappings(save_root, src_pos_list_np, max_xy_list_np, src_img_PIL, tgt_img_PIL, heatmap=None)
    visualize_mappings(save_root, src_pos_list_np, max_xy_list_np, src_img_PIL, tgt_img_PIL, heatmap=relevance_map_ls)
...
if __name__ == '__main__':
    src_img_path = '/home/qianxu/Project25/Diffusion-3D-Features/test/feature_output/001_cm/texture_images/Ceramic_0.png'
    tgt_img_path = '/home/qianxu/Downloads/shovel/CL8R2410029/colors.png'
    src_img_PIL = Image.open(src_img_path).convert('RGB')
    tgt_img_PIL = Image.open(tgt_img_path).convert('RGB')
    prompt = 'Grasp a brush'
    src_pos_list = [(280, 220), (200, 210), (250, 250)]
    save_root = '/home/qianxu/Project25/RAM_code/test'
    map_keypoints(src_img_PIL, tgt_img_PIL, prompt, src_pos_list, save_root=save_root, ftype='sd_dinov2')
    print('done')