...
def visualize_mappings(save_root, src_pos_ls, tgt_pos_ls, src_img_PIL, tgt_img_PIL, heatmap=None, filename=None):
    '''
    Visualize source and target point mappings with optional heatmaps.
    - If heatmap is None, plot all points with labels on one figure.
    - If heatmap is provided, plot separate figures for each point with heatmap overlay.
    '''
    ...
    if heatmap is None:
        ...
        axes[0].set_title('Source Image')
        axes[0].axis('off')
        for idx, (x, y) in enumerate(src_pos_ls):
            axes[0].scatter(x, y, c='r', marker='x')
            axes[0].text(x + 5, y + 5, str(idx + 1), color='white', fontsize=12, weight='bold')

        axes[1].imshow(tgt_img_PIL)
        axes[1].set_title('Target Image')
        axes[1].axis('off')
        for idx, (x, y) in enumerate(tgt_pos_ls):
            axes[1].scatter(x, y, c='r', marker='x')
            axes[1].text(x + 5, y + 5, str(idx + 1), color='white', fontsize=12, weight='bold')

        save_path = f'{save_root}/{filename}.png' if filename else f'{save_root}/mappings_overview.png'
        plt.savefig(save_path)
        plt.close()
    else:
        for idx, ((sx, sy), (tx, ty)) in enumerate(zip(src_pos_ls, tgt_pos_ls)):
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            axes[0].imshow(src_img_PIL)
            axes[0].scatter(sx, sy, c='r', marker='x')
            axes[0].text(sx + 5, sy + 5, str(idx + 1), color='white', fontsize=12, weight='bold')
            axes[0].set_title(f'Source Point {idx + 1}')
            axes[0].axis('off')

            # heatmap_resized = np.array(Image.fromarray(heatmap[idx]).resize(tgt_img_PIL.size, Image.BILINEAR))
            heatmap_resized = np.array(Image.fromarray(heatmap[idx]))
            heatmap_normalized = (heatmap_resized - np.min(heatmap_resized)) / (np.max(heatmap_resized) - np.min(heatmap_resized))
            axes[1].imshow(tgt_img_PIL)
            axes[1].imshow(255 * heatmap_normalized, alpha=0.45, cmap='viridis')
            axes[1].scatter(tx, ty, c='r', marker='x')
            axes[1].text(tx + 5, ty + 5, str(idx + 1), color='white', fontsize=12, weight='bold')
            axes[1].set_title(f'Target Point {idx + 1} with Heatmap')
            axes[1].axis('off')

            save_path = f'{save_root}/{filename}_{idx + 1}.png' if filename else f'{save_root}/mapping_{idx + 1}.png'
            plt.savefig(save_path)
            plt.close()
