...
class HandModelMJCF:
    def __init__(self, mjcf_path, mesh_path=None, n_surface_points=2000, device=None,
        ...
        if mjcf_path.endswith('.urdf'):
            self.chain = pk.build_chain_from_urdf(open(mjcf_path).read()).to(dtype=torch.float, device=self.device)
        else:
            ...
            if (len(body.link.visuals) > 0):
                ...
                for visual in body.link.visuals:
                    ...
                    if visual.geom_type == "box":
                        ...
                        link_mesh = trimesh.load_mesh(
                            os.path.join(mesh_path, visual.geom_param), process=False)
        ...
        def set_joint_range_recurse(body):
            if body.joint.joint_type != "fixed":
                ...
                self.joints_lower.append(body.joint.range[0] if body.joint.range is not None else torch.tensor(-3.14, dtype=torch.float, device=self.device))
                self.joints_upper.append(body.joint.range[1] if body.joint.range is not None else torch.tensor(3.14, dtype=torch.float, device=self.device))
            for children in body.children:
    ...
    def set_parameters(self, hand_pose, retarget:bool = True, robust:bool = True, contact_point_indices=None):
        ...
        # 'robot0:THJ1', 'robot0:THJ0']
        # if use_axis_angle:
        #     rotate_matrix = Rotation.from_rotvec(hand_pose[:, 3:6].cpu().numpy()).as_matrix()
        #     rotate_matrix = torch.tensor(rotate_matrix, dtype=torch.float, device=self.device)
        #     self.hand_pose = torch.cat([hand_pose[:, :3], rotate_matrix[:, 0], rotate_matrix[:, 1], hand_pose[:, 6:]], dim=-1)
        # else:
...
class HandModelURDF:
    def __init__(self, robot_name:str, urdf_path=None, mesh_path=None, device=None):
        ...
        self.chain = pk.build_chain_from_urdf(open(urdf_path).read()).to(dtype=torch.float, device=self.device)
        print(self.chain.get_joint_parameter_names())
        self.n_dofs = len(self.chain.get_joint_parameter_names())
    ...
    def set_qpos(self, qpos):
        ...
        self.global_translation = self.hand_pose[0:3]
        self.global_rotation = axis_angle_to_matrix(self.hand_pose[3:6])
