...
def fit_capsule_to_points_optimization(points: torch.Tensor, padding_factor: float = 1.05,
                                      num_iterations: int = 1200, learning_rate: float = 0.01,
                                      lambda_r: float = 5e-5, lambda_len: float = 5e-5, alpha = 1e-3):
    """
...
class HandRobotWrapper:
    ...
    def __init__(self, robot_name: str, urdf_path: str, mesh_path: str, device=None,
        ...
        approx_params = json.load(open(approx_params_path, "r"))
        ...
        self.link_approx_names = {
            ...
            "allegro_hand": ["link_14.0", "link_15.0", "link_1.0", "link_2.0", "link_5.0", "link_6.0", "link_9.0", "link_10.0", "link_3.0_tip","link_7.0_tip","link_11.0_tip","link_15.0_tip"],
        ...
        def build_mesh_recurse(body):
            ...
            if len(vvisuals) > 0:
                ...
                if link_name in self.link_approx_names:
                    ...
                    if COMPUTE_APPROX:
                        pcd = o3d.geometry.TriangleMesh.sample_points_uniformly(link_mesh, number_of_points=100) # 采样点云
                        points = np.asarray(pcd.points)
                        capsule_params = fit_capsule_to_points_optimization(torch.from_numpy(points).to(dtype=torch.float))
                        print(f'"{link_name}"',": ", {k:v.detach().numpy().tolist() for k,v in capsule_params.items()}, ",")
                    else:
                        capsule_params = {k:torch.tensor(v) for k,v in approx_params[link_name].items()}
        ...
        # ---------------------------------------------------------------------- #
        if tip_aug:
            fingertip_link_names = [k for k in areas.keys() if k in self.tip_names]
            middle_link_names = [k for k in areas.keys() if k in self.link_approx_names and k not in self.tip_names]
            for name in fingertip_link_names: areas[name] *= tip_aug
            for name in middle_link_names: areas[name] *= tip_aug * 0.8
        
        if robot_name == 'shadow_hand': areas['forearm'] = 0
